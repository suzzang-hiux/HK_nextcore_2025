      ...
      return (ret);
		...
		this._edit = function (obj, col, element) {
			...
			var rtl = this._data.core.rtl,
                ...
                this.element.trigger('update_cell.jstree-grid', { node: obj, col: col.value, value: v, old: t, instance : this, sourceName : col.sourceName });
                var id = _guid();
                this._detachColumns(id);
				...
				return function(e) {
					...
					var event = jQuery.Event("select_cell.jstree-grid");
					 tree.trigger(event, [{value: val,column: col.header,node: node,grid:$(this),sourceName: col.value
						, objData : objData, col : col, instance:__this, e : e}]);
        ...
        for (var i = 0, len = cols.length;i<len;i++) {
          ...
          gridCellParent = findDataCell(uniq, gridCellParentId, i, column);
					...
					tree.trigger("render_cell.jstree-grid", [{value: val, column: col.header, node: t, sourceName: col.value, last : last, instance : this, col : col}]);
				}
