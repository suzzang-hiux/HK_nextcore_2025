...
if (window.jQuery)(function ($) {
		...
		this
			...
			.each(function () {
				...
				if (!MultiFile.list) {
					...
					MultiFile.wrapper.append('<br><div class="img_se_wrap" id="' + MultiFile.wrapID + '_list"></div>');
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					//var names = $('</span>');
					var names = $('<div style="border:0px">');
					$.each(files, function (i, file) {
					...
					//$.each(files, function (i, file) {
						...
						var
							...
							r = $('<div style="padding:0px"></div>'),	
							//b = $('<a class="MultiFile-remove" href="#' + MultiFile.wrapID + '">' + MultiFile.STRING.remove + '</a>')
							b = $('<a class="MultiFile-remove fa-solid fa-circle-xmark" href="#' + MultiFile.wrapID + '"></a>')		
								// ********
								// TODO:
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		maxfile: 4082, // maximum size of a single file
		...
		STRING: {
			remove: 'x',
			denied: '$ext 파일은 등록 할 수 없는 형식의 파일 입니다.',
			file: '$file',
			selected: 'File selected: $file',
			duplicate: 'This file has already been selected:\n$file',
			toomuch: 'The files selected exceed the maximum size permited ($size)',
			toomany: 'Too many files selected (max: $max)',
			toobig: '$file 용량이 큽니다(4M 이내 가능) (max $size)'
		},
		...
		error: function (s) {
			...
			alert(s);
			//top.pencakeMainPageNew.showAlertMessage(s, function(){top.pencakeMainPageNew.closeAlertMessage();});
		}
	}; //} });
