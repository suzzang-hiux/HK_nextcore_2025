 ...
 * @module PubEP
 * @author PentaSystems
 * @property {Function} init  [init]{@link module:PubEP~init}
 * @property {Function} getOption  [getOption]{@link module:PubEP~getOption}
 * @property {Function} getContextPath  [getContextPath]{@link module:PubEP~getContextPath}
 * @property {Function} getPojectName  [getPojectName]{@link module:PubEP~getPojectName}
 * @property {Function} cookie  [cookie]{@link module:PubEP~cookie} 
 * 								<p style="color:red">※ PENCAKE 쿠키 정보 공통 함수
 * 							    <p>쿠키 조회 : PubEP.cookie("키")
 * 							    <p>쿠키 설정 : PubEP.cookie("키", "값", {옵션})
 * 
 * @property {Object} req
 * @property {Function} req.ajax  [ajax]{@link module:PubEP~ajax}
 * 								<p style="color:red">※ PENCAKE AJAX 호출 공통 함수, JQUERY OPTION 동일
 * 							    <p>AJAX 호출 : PubEP.req.ajax({옵션})
 * @property {Function} req.resultMessage  [resultMessage]{@link module:PubEP~resultMessage}
 * @property {Function} req.getParam  [getParam]{@link module:PubEP~getParam}
 * @property {Function} req.formData  [formData]{@link module:PubEP~formData}


 * @property {Function} log   [log]{@link module:PubEP~log}
 * 								<p style="color:red">※ PENCAKE 자바스크립트 디버깅 로그, console.log 동일 파라메터
 * 							    <p>PubEP.log(파라메터)


 * @property {Function} logWrite   [logWrite]{@link module:PubEP~logWrite}
 * 								<p style="color:red">※ PENCAKE 로그
 *							    <p>PubEP.logWrite(firstItem.PAGE_ID, '' ,{gubun:'main_screen' ,gubunkey: firstItem.PAGE_ID});
 * 
 * @property {Object} page
 * @property {String} page.popupPostMsg
 * @property {Function} page.view   [view]{@link module:PubEP~view}
 * 								<p style="color:red">※ PENCAKE 페이지 열기
 *								<p> popup ex : PubEP.page.view("http://dev.pub.com/",'popup',{gubun:'menu', gubunkey:'menu_pub',name:'popup name', method:'get or post',viewOption:'toolbar=yes, scrollbars=yes, resizable=yes, top=500, left=500, width=400, height=400'});
								<p> location ex : PubEP.page.view("http://dev.pub.com/",'location',{gubun:'menu', gubunkey:'menu_location'});

 * @property {Function} page._viewPage   [_viewPage]{@link module:PubEP~_viewPage}
 * @property {Function} page._location   [_location]{@link module:PubEP~_location}
 * @property {Function} page._popup   [_popup]{@link module:PubEP~_popup}
 * @property {Function} page._iframe   [_iframe]{@link module:PubEP~_iframe}
 * 
 * @property {Function} download
 * 
 * @property {Object} util
 * @property {Function} util.calcDate   [calcDate]{@link module:PubEP~calcDate}
 * @property {Function} util.convertCamel   [convertCamel]{@link module:PubEP~convertCamel}
 * @property {Function} util.charFill   [charFill]{@link module:PubEP~charFill}
 * @property {Function} util.domain   [domain]{@link module:PubEP~domain}
 * @property {Function} util.objectMerge   [objectMerge]{@link module:PubEP~objectMerge}
 * @property {Function} util.dateFormat   [dateFormat]{@link module:PubEP~dateFormat}
 * @property {Function} util.removeSpecial   [removeSpecial]{@link module:PubEP~removeSpecial}
 * @property {Function} util.removeMoneySpecial   [removeMoneySpecial]{@link module:PubEP~removeMoneySpecial}
 * @property {Function} util.setRangeDate   [setRangeDate]{@link module:PubEP~setRangeDate}
 * @property {Function} util.functionCall   [functionCall]{@link module:PubEP~functionCall}
 * @property {Function} util.generateUUID   [generateUUID]{@link module:PubEP~generateUUID}
 * @property {Function} util.popupPosition   [popupPosition]{@link module:PubEP~popupPosition}
 * @property {Function} util.replaceUrl   [replaceUrl]{@link module:PubEP~replaceUrl}
 * @property {Function} util.replaceParamUrl   [replaceParamUrl]{@link module:PubEP~replaceParamUrl}
 * @property {Function} util.replaceParam   [replaceParam]{@link module:PubEP~replaceParam}
 * @property {Function} util.allAttr   [allAttr]{@link module:PubEP~allAttr}
 * @property {Function} util.allAttrNameLower   [allAttrNameLower]{@link module:PubEP~allAttrNameLower}
 * @property {Function} util.allAttrNameUpper   [allAttrNameUpper]{@link module:PubEP~allAttrNameUpper}
 * @property {Function} util.getParameter   [getParameter]{@link module:PubEP~getParameter}
 * @property {Function} util.getQueryParameter   [getQueryParameter]{@link module:PubEP~getQueryParameter}
 * @property {Function} util.setQueryParameter   [setQueryParameter]{@link module:PubEP~setQueryParameter}
 * @property {Function} util.getTreeModel   [getTreeModel]{@link module:PubEP~getTreeModel}
...
(function (window, $) {
  ...
  var _$base = {},
    ...
    globalOption = {
      ...
      */
      ...
      beforePageView: function (callback) {
        callback(true);
      },
   ...
   * @desc PubEP.req request 처리를 위한 모듈
   */
  _$base.req = {
    /**
     * @method ajax
    ...
    ajax: function (option, formData) {
      ...
      //20191126 KEA
      if (ajaxOpt.success) {
        var _success = ajaxOpt.success;

        ajaxOpt.success = function (res) {
          if (res.status && res.status == "login error") {
            //location.href = "/pencake/oauth";
            location.href = "/pencake/error.jsp?status=relogin";
     ...
     * @param {boolean} errorMsgView
     * @desc request 에대한 result 에러 내용 보기.
     */
  ...
  jQuery.fn.centerLoading = function (options) {
    ...
    if (config.content) {
      if (config.centerYn == "Y") {
        loadStr += '<div style="z-index:10;margin: 0;position: absolute; top: 50%;left: 50%;transform: translate(-50%, -50%);">' + config.content + "</div>";
      } else {
        loadStr += config.content;
      }
    } else {
      loadStr += '<div style="z-index:10;position: absolute;top: 50%;left: 50%;transform: translate(-50%, -50%);"><img src="' + config.loadingImg + '"/></div>';
    }
    loadStr += "</div>";
   ...
   * @param {String} type
   * @param {Object} options
   * @desc url open 메소드 view 필수 항복  url, type , options{gubun:'menu , portlet, sso 등등', gubunkey:'구분키 값' ,beforeCheck :''}
   * @example
   ...
   *
   */
  _$base.logWrite = function (url, type, options, callbackSuccess, callbackError) {
    if (globalOption.log.enabled !== true) {
      return;
    }

    var tmpInfo = typeof pubEPortalConfig === "undefined" ? { replaceParam: { userid: "" } } : pubEPortalConfig;
    var _gubun = options.gubun;
    if (options.logWriteFlag !== false && (globalOption.log.logWriteKey == "all" || $.inArray(options.gubun, globalOption.log.logWriteKey) > -1)) {
      _gubun = _gubun || "";
      var _gubunkey = options.gubunkey || "";

      if (globalOption.log.isBlankAllow === false && (_gubun == "" || _gubunkey == "")) {
        return false;
      }
  ...
  _$base.page = {
     ...
     * @desc 페이지
     */
    view: function (url, pType, options, postMessage) {
    ...
    _popup: function (url, options) {
      ...
      if (tmpPopOption != "") {
        ...
        var tmpItem,
          ...
          addFlag,
          tmpOpt = "",
          addScrollbarOpt = true,
          addStatusOpt = false,
          addResizeableOpt = true;
    ...
    _iframe: function (url, options) {
      ...
      if (options.method == "post") {
        ...
        if (options.onlySrc) {
          ...
          tmpiframe[0].contentWindow.location.replace(openUrl);
        } else {
          // tmpiframe.off("load");
  ...
  _$base.util = {
     ...
     * @desc char camel case 로 변경. user_name -> userName
     */
    ...
    /**
     * @method functionCall
     * @param {Object} fnInfo {Strinb, Object {name :'functionName' , context : 'call object ex)window, PubEP'}}
     * @desc 문자열 function 호출.
     */
    ...
    replaceParam: function (str, replaceParam) {
      ...
      if (matchObj != null) {
        var _paramVal = str,
          ...
          matchKey,
          orginKey,
          paramObjFlag = typeof replaceParam === "object";
    ...
    /**
     * @method allAttrNameLower
     * @param attrObj jquery object
     * @desc attribute 소문자 이름으로 리턴
     */
    allAttrNameLower: function (attrObj) {
      return this.allAttr(attrObj, "lower");
    },
    /**
     * @method allAttrNameUpper
     * @param attrObj jquery object
     * @desc attribute 대문자 이름으로 리턴
     */
